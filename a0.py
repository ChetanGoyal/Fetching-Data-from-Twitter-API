...
consumer_key = '3OfMgrWrNlvhVWGJK6lsCdHl7'
consumer_secret = 'K4RP4sL0oqSM9QY3QFGqEVJ78aleT0e1uBJuuhWjH775ZLCVPe'
access_token = '717807760226787329-WHjUFNETqYxZgl50k1ColmBca3MC9IN'
access_token_secret = 'BoKoe4WB37ftMZzwS4z8gbqMUGMmpaJSoJzUTh0JPTCBl'
...
candidates=read_screen_names('candidates.txt')
    #pass
...
def count_friends(users):
    ...
    friend_all=[]
    for i in users:
        friend_all+=i["friends"]
    c=Counter(friend_all)
    return c
    pass
...
def friend_overlap(users):
    ...
    for i in range(0,len(users)):
        for j in range(i+1,len(users)):
            user1=set(users[i]["friends"])
            user2=set(users[j]["friends"])
            overlap=user1.intersection(user2)
            overlap_friend.append((users[i]["screen_name"],users[j]["screen_name"],len(overlap)))
    return (sorted(overlap_friend,reverse=True,key=lambda x: x[2]))
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    common_list=[]
    for u in users:
        if u["screen_name"]=="HillaryClinton" or u["screen_name"]=="realDonaldTrump":
            common_list+=u["friends"]
    c=Counter(common_list).most_common()
    for i in c:
        if i[1]>1:
            request= robust_request(twitter,'users/lookup',{'user_id': i[0]})
            a=eval(request.text.replace(":false",":False").replace(":true",":True").replace(":null",":None"))
    for i in a:
...
def create_graph(users, friend_counts):
    ...
    G = nx.Graph()
    for cand in users:
        G.add_node(cand['screen_name'])
        for frnd in cand['friends']:
    ...
    for cand in users:
        for frnd in cand["friends"]:
            if friend_counts[frnd]>1:
                G.add_edge(cand['screen_name'],frnd)
...
def draw_network(graph, users, filename):
    ...
    for u in users:
        for node in graph.nodes():
            if str(node) in u['screen_name']:
                labels[node] = node
    plt.figure(figsize=(12,12))
    nx.draw_networkx(graph,node=node,labels=labels,alpha=.5, width=.1,node_size=100)
    plt.axis("off")
